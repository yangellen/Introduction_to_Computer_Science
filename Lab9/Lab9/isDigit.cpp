...
//take a string to check if it's all letters and return bool
bool isLetter(string choice)
{
   int count = 0;
   bool letter = true;
   for (int i = 0; i < choice.length(); i++) //for each char in string
   {
      ...
      if (count == choice.length())
      {
         letter = true; //all letters
      }
      else if (count != choice.length())  //not all letter
      {
         letter = false;
      }
   }
